// Package httpapi exposes go-be's operational surface: /livez, /readyz (gated on the first
...
func NewMux(m *lifecycle.Manager, quoteHandler, blockedRoutesHandler http.HandlerFunc) *http.ServeMux {
	...
	mux.HandleFunc("GET /metrics", func(w http.ResponseWriter, _ *http.Request) {
		...
		fmt.Fprintf(w, "go_be_prepared %d\n", prepared)
		fmt.Fprintf(w, "go_be_uptime_seconds %.0f\n", m.Uptime().Seconds())
		fmt.Fprintf(w, "go_be_snapshot_reloads_total %d\n", m.Reloads.Load())
		fmt.Fprintf(w, "go_be_snapshot_load_errors_total %d\n", m.LoadErrors.Load())
		...
		fmt.Fprintf(w, "go_be_request_path_mysql_ops_total 0\n")
		delegate.Metrics(w)
		if snap != nil {
			fmt.Fprintf(w, "go_be_snapshot_age_seconds %.0f\n", time.Since(snap.LoadedAt).Seconds())
			fmt.Fprintf(w, "go_be_snapshot_pe_version{version=%q} 1\n", snap.PEVersion)
			...
			for _, t := range tables {
				fmt.Fprintf(w, "go_be_snapshot_rows{table=%q} %d\n", t, snap.Rows[t])
			}
		}
	})
